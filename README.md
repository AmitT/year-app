...
1. uncomment **line 31** in `MainActivity.java` and change `http://yearyear.com` to match your remote source

	```
	mWebView.loadUrl("http://yearyear.com");
	```
...
3. open the `MyAppWebViewClient.java` file and replace `yearyear.com` in **line 12** with your custom url

	```
	if (Uri.parse(url).getHost().endsWith("yearyear.com")) {
	```
